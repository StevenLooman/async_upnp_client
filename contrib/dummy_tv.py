...
class StateVariable(object):
    ...
    def __str__(self):
        return "<{}({}, {})>".format(self.__class__.__name__, self.SERVICE_NAME, self.value)
...
def async_handle_control(request, state_variables, xml_ns):
    ...
    # return result
    response_base = '<?xml version="1.0" encoding="utf-8"?>' \
...
def async_handle_subscribe_rc(request):
    response = yield from async_handle_subscribe(request, SUBSCRIBED_CLIENTS['RC'], STATE_VARIABLES['RC'])
    return response
...
app.router.add_route('UNSUBSCRIBE', '/upnp/event/AVTransport1', async_handle_unsubscribe_avt)
